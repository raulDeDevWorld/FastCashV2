...
export default function Navbar({ children }) {
    const { user, userDB, setUserProfile, nav, setNav, userNav, navItem, setNavItem, setUserNav, state, setState, setUserSuccess, success, setUserData, postsIMG, setUserPostsIMG, divisas, setDivisas, setCountries, enviosDB, cambiosDB, notificaciones, setNotificaciones, setEnviosDB, setCambiosDB } = useAppContext()
                ...
                {((enviosDB && enviosDB !== undefined) || (cambiosDB && cambiosDB !== undefined)) && Object.values(enviosDB && enviosDB !== undefined && cambiosDB && cambiosDB !== undefined ? { ...enviosDB, ...cambiosDB } : (enviosDB && enviosDB !== undefined ? enviosDB : (cambiosDB && cambiosDB !== undefined ? cambiosDB : {}))).filter((i) => i.notificaciones !== undefined && i.notificaciones === true).length > 0 ? <ul> {Object.values({ ...enviosDB, ...cambiosDB }).filter((i) => i.notificaciones !== undefined && i.notificaciones === true).sort((a, b) => b.date - a.date).map((i, index) => {
                    return <li className='relative pb-4 border-b-[1px] border-gray-300 text-black' >
                        <span className='w-full pr-[10px]'>Tu {i.operacion} de dinero de
                            ...
                            {i.estado == 'Verificado' && <span className={` text-black  bg-blue-400`}>esta VERIFICADO, la transaferencia esta apunto de realizarse.</span>}
                            {i.estado == 'Transfiriendo' && <span className={` text-black  bg-yellow-400`}>'ya se esta transfiriendo</span>}
                            {i.estado == 'Exitoso' && <span className={` text-black bg-green-400`}>ha sido exitoso</span>}
                            {i.estado == 'Rechazado' && <span className={` text-black bg-red-400 `}>ha sido rechazado'</span>}
                        </span>
